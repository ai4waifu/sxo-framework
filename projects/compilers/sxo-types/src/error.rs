//! Product-layer error; wraps Athena diagnostics when present.
...
pub struct SxoError {
    ...
    pub message: String,
    /// Stable Athena code (`ATHENA_*`) when the failure originated in the kernel.
    pub athena_code: Option<String>,
}

impl SxoError {
    /// Create a product error without a kernel code.
    pub fn new(message: impl Into<String>) -> Self {
        Self { message: message.into(), athena_code: None }
    }
...
impl std::fmt::Display for SxoError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(code) = &self.athena_code { write!(f, "{code}: {}", self.message) } else { write!(f, "{}", self.message) }
    }
}
