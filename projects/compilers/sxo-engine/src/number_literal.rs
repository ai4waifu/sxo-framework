//! Frontend number literal parse/render (SXO-owned; Euler holds [`Number`] only).

use euler::{ExactNumber, Number, RealNumber};
...
pub fn parse_number_literal(text: &str) -> Option<Number> {
    ...
    let is_machine = source.contains('.')
        || source.contains('e')
        || source.contains('E')
        || (source.ends_with('.') && source.len() > 1);
    if is_machine {
